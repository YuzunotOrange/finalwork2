...
int find_flow_arg_path( int N, int **adjacent, int **capacity, int **flow,
  ...
  /*
    ...
    (1) adjacent[][]を利用して，深さ（または幅）優先探索を実施．
      ...
      ・ parent[]を利用して，深さ（または幅）優先探索木を作成する．
      ・ 親となる点がない場合には，parentの値を-1としておく．
    (2) parent[]を辿って，sからtへの点列をpath[]に格納する．
      ・ parentはtからsへの方向にのみ辿れることに注意せよ．
      ・ 例えば，フロー増加道が 0 -> 1 -> 8 -> 9 の場合，
         path[0]=0, path[1]=1, path[2]=8, path[3]=9 となる．
	 それ以外の要素には特に何も入れなくて良い．
	 （path[]の終わりは，値がtとなることで確認できるため．）
    (3) フロー増加道が見つからなかった場合には，parent[]を辿っている
  ...
  */
  ...
  visit(N, adjacent, capacity, flow, s, YetToVisit, parent);
