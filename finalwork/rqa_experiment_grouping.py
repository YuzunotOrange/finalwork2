# 2025-11-05
# rqa_experiment_grouping.py
# 実行方法: python rqa_experiment_grouping.py
# new_li/ 内の全CSVを自動解析し、rqa_result/ にRQA結果を保存（TT含む完全指標）
# DET は別グラフとして保存
#解析結果のグルーピングも実装
#決定論性に着目し降順にソートすることで決定論と確率論が自然に二分される。
...
def rqa_measures_full(R, lmin=2, vmin=2, wmin=2):
    ...
    return {
        "RR": rr, "DET": det, "L": L, "L_max": Lmax, "DIV": DIV, "L_entr": L_entr,
...
def plot_sorted_bar(df, key="mD", path="rqa_result/sorted_bar.png"):
    ...
    if "group" in df.columns:
        ...
        colors = groups.cat.codes #自動で色分け
        plt.bar(range(len(df)), df[key].astype(float).values, color=plt.cm.tab20(colors))
        handles = []
        #グループごとに凡例を出すため、散布でダミーを作成
        for g in groups.cat.categories:
            handles.append(plt.Rectangle((0,0), 1, 1,
                                        color=plt.cm.tab20(groups.cat.categories.get_loc(g))))
        plt.legend(handles, list(groups.cat.categories), title="Group", loc="best")
    else:
...
def plot_rqa_param_space(df, out_dir=MAIN_OUT_DIR,
    ...
    ax.scatter(d["mD"].astype(float), d["mL"].astype(float), d["mEN"].astype(float),
                    c=colors, s=40, depthshade=True)

    ax.set_xlabel("mD (mean DET)")
    ax.set_ylabel("mL (mean L)")
    ax.set_zlabel("mEN (mean ENTR)")
    ax.set_title("RQA Parameter Space (mD, mL, mEN)")

    # 凡例（グループごと）
    handles = [plt.Line2D([0],[0], marker="o", linestyle="",
                                  color=cmap(groups.cat.categories.get_loc(g) % 20), label=str(g))
                for g in groups.cat.categories]
    ...
    plt.close(fig)
        ...
        # ===== 2D ペア散布図 =====
    base_pairs = [("mD","mL"), ("mD","mEN"), ("mL","mEN")]
    out_full = os.path.join(out_dir, path2d)

    # --- 拡大なし（全体）
    scatter_pairs(d, base_pairs, colors, groups, cmap,
                out_path=out_full,
                title="RQA Parameter Space (2D pairs)")

    # --- 拡大あり（指定レンジ）
    zoom_ranges = {
        ("mD","mL"):  {"xlim": (0.7, 1.0), "ylim": (0, 50)},
        ("mD","mEN"): {"xlim": (0.8, 1.0), "ylim": (1.5, 4)},  # 必要に応じて (2,3) などへ
    ...
    # --- TT の図（別ファイル）
    if "mTT" in d.columns:
        tt_pairs = [("mD","mTT"), ("mL","mTT"), ("mEN","mTT")]
        out_tt_full = os.path.join(out_dir, "rqa_param_space_pairs_TT.png")
        scatter_pairs(d, tt_pairs, colors, groups, cmap,
                    out_path=out_tt_full,
                    title="RQA Parameter Space (TT pairs)")
        
        #---拡大図
        tt_zoom_range = {
            ("mD", "mTT"): {"xlim": (0.7, 1.0), "ylim":(0, 40)},
            ("mL", "mTT"): {"xlim": (0, 20), "ylim": (0, 40)},
    ...
    # === CSV保存はそのまま ===
    cols = ["file","group","rank","mD","mL","mEN","mTT"]
...
def process_csv(path, eps_min, eps_max, eps_steps, lmin):
    ...
    if X.size>0:
        ...
        pd.DataFrame([full]).to_csv(os.path.join(out_dir,f"full_measures_rr5_{base}.csv"),index=False)

    pd.DataFrame([{
        "file":os.path.basename(path),"tau":tau,"m":m,
        "mD":avg["mD"],"mL":avg["mL"],"mEN":avg["mEN"],
        "mTT" : mTT
    }]).to_csv(os.path.join(out_dir,f"result_summary_{base}.csv"),index=False)

    return {"file":os.path.basename(path),"tau":tau,"m":m,**avg, "mTT": mTT}

def scatter_pairs(d, pairs, colors, groups, cmap, out_path,
...
def main():
    ...
    # ---- 追加　ソート&グループ設定 ----
    parser.add_argument("--sort_key", choices=["mD", "mL", "mEN", "mTT"], default="mD", help="集計ソート（降順)")
    parser.add_argument("--group_method", choices=["tertile", "quartile", "threshold"], default="tertile", help="グルーピング手法")
    ...
    if args.group_method == "threshold":
        if args.threshold.strip():
            ...
            # 例: デフォルト境界（任意）— mD の経験値がなければ後から調整してください
            thresholds = [0.4, 0.25]
    df_grouped = assign_group(df_sorted, key=key, method=args.group_method, thresholds=thresholds)
    ...
    plot_sorted_bar(df_grouped, key=key, path=os.path.join(MAIN_OUT_DIR, f"sorted_bar_{key}.png"))
    # RQAパラメータ空間の散布図（3D & 2D）
    plot_rqa_param_space(df_grouped, out_dir=MAIN_OUT_DIR,
                     path3d="rqa_param_space_3d.png",
                     path2d="rqa_param_space_pairs.png")

    print(f"\n[Saved] {MAIN_OUT_DIR}/sorted_bar_{key}.png")
