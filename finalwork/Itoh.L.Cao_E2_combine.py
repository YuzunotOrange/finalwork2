...
def main():
    ...
    parser.add_argument("--output_dir", default="Itho.E2_result", help="Directory to save outputs")
    ...
    lorenz = load_series_whitespace_firstcol(lorenz_path)
    print("[INFO] Lorenz: tau fixed = 5")

    # Lorenz curves（短い表示名 = 凡例とは別に参照ボックスで使用）
    e2_curves_common = []
    ...
    e2_curves_common.append(
        compute_e2_for_series(
            lorenz + noise, args.max_dim, tau=5, k=args.k_nn, theiler=args.theiler,
    ...
    for f in files:
        ...
        curves.append(
            compute_e2_for_series(
                x_raw, args.max_dim, tau=tau1, k=args.k_nn, theiler=args.theiler,
