...
for fname in csv_files:
    try:
        ...
        data_points = (data_points - data_min) / (data_max - data_min)

        #amplitude is now 1.0
        amplitude = np.max(data_points) - np.min(data_points) #should be 1.0
        radius = 0.1 * amplitude
        print(f"Normalized data. Amplitude: {amplitude:.5f}, Radius:{radius:.5f}")
     ...
     # Output
        imagename = os.path.join(output_folder, f"{name}_{tau}_rp-sigma.png")
        outname = os.path.join(output_folder, f"{name}_{tau}_rqa-sigma.txt")
        print(result)
