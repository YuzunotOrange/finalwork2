#2025-10-21
#決定論性のグラフにおけるεの小さいところと大きい部分のリカレンスプロットの比較

"""
決定性カーブの両端のεで比較
python rqa_experiment.py data.csv --eps-min 0.1 --eps-max 2.0 --rp-compare

任意の２点のεで比較
python rqa_experiment.py data.csv --rp-compare --eps-small 0.3 --eps-large 1.2
"""
...
def main():
    ...
    ap.add_argument("--rp-compare", action="store_true", help="εの小さいRPと大きいRPを横並びで比較保存用")
    ap.add_argument("--eps-small", type=float, default=None, help="RP比較用の小さいε（未指定であれば--eps_min")
    ap.add_argument("--eps-large", type=float, default=None, help="RP比較用の大きいε（未指定であれば--eps_max")
    args = ap.parse_args()
    ...
    # RP（任意）
    if args.rp_compare and X.size > 0:
        try:
            eps_small = args.eps_small if args.eps_small is not None else args.eps_min
            eps_large = args.eps_large if args.eps_large is not None else args.eps_max

            #再帰行列を作成
            R_small = recurrence_matrix(X, epsilon=float(eps_small), normalize=True)
            R_large = recurrence_matrix(X, epsilon=float(eps_large), normalize=True)
            rr_small = float(R_small.mean())
            rr_large = float(R_large.mean())

            #横並び図
            plt.figure(figsize=(10, 5))
            plt.subplot(1, 2, 1)
            plt.imshow(R_small, origin='lower', cmap='gray_r', interpolation='none')
            plt.title(f"RP (ε={eps_small:.3f}, RR={rr_small:.3f})")
            plt.xlabel("i")
            plt.ylabel("j")

            plt.tight_layout()
            rp_compare_path = os.path.join(out_dir, f"rp_compare_small_large_{base}.png")
            plt.savefig(rp_compare_path, dpi=200)
            plt.close()
            print(f"[Saved] {rp_compare_path}")

            #単独RPを出力する部分
            rp_small_path = os.path.join(out_dir, f"rp_small_eps_{eps_small:.3f}_{base}.png")
            plt.figure(figsize=(5,5))
            plt.imshow(R_small, origin='lower', cmap='gray_r', interpolation='none')
            plt.title(f"RP (ε={eps_small:.3f}, RR={rr_small:.3f})")
            plt.xlabel("i"); plt.ylabel("j"); plt.tight_layout()
            plt.savefig(rp_small_path, dpi=200); plt.close()
            print(f"[Saved] {rp_small_path}")
            
            rp_large_path = os.path.join(out_dir, f"rp_large_eps_{eps_large:.3f}_{base}.png")
            plt.figure(figsize=(5,5))
            plt.imshow(R_large, origin='lower', cmap='gray_r', interpolation='none')
            plt.title(f"RP (ε={eps_large:.3f}, RR={rr_large:.3f})")
            plt.xlabel("i"); plt.ylabel("j"); plt.tight_layout()
            plt.savefig(rp_large_path, dpi=200); plt.close()
            print(f"[Saved] {rp_large_path}")

        except Exception as e:
            print(f"[WARN] RP比較の保存に失敗")
