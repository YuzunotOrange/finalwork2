...
for name, base_data in datasets.items():
    for rate in missing_rates:
        print(f"\n--- Analyzing {name} with Missing Rate {int(rate*100)}%---")

        #データを破損させて、線形補間で埋める処理
        data, mask = introduce_block_missing_and_interpolate(base_data, missing_rate=rate, block_len=10)
        ...
        plt.title(f"{name} (Missing Rate: {int(rate*100)}%)")
